...
void main() async {
  ...
  final eventCategories = [
    'Экшен', 'Приключения', 'Головоломки', 'Стратегии', 'Ролевые',
    'Казуальные', 'Спортивные', 'Гонки', 'Файтинг', 'Симуляторы',
    'Выживание', 'Фэнтези', 'Открытый мир', 'Многопользовательская',
    'Хоррор', 'Зомби', 'Инди'
  ];
  ...
  for (int i = 1; i <= 100; i++) {
    ...
    if (isAction) {
      ...
      bannerData["eventCategory"] = faker.randomGenerator.element(eventCategories);
      bannerData["eventDescription"] = faker.randomGenerator.element(eventDescriptions);
      eventCounter++;
    }
